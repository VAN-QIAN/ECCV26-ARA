...
_LOCAL_MODEL_PATH = os.path.join(os.path.dirname(__file__), 'bem_model')
_REMOTE_MODEL_PATH = 'https://tfhub.dev/google/answer_equivalence/bem/1'


def _default_bem_model_path():
  saved_model_path = os.path.join(_LOCAL_MODEL_PATH, 'saved_model.pb')
  variables_path = os.path.join(_LOCAL_MODEL_PATH, 'variables', 'variables.index')
  if os.access(saved_model_path, os.R_OK) and os.access(variables_path, os.R_OK):
    return _LOCAL_MODEL_PATH
  return _REMOTE_MODEL_PATH


_MODEL_PATH = os.environ.get('EVQA_BEM_MODEL_PATH', _default_bem_model_path())
...
def initialize_bem_scoring_function(vocab_path=_VOCAB_PATH,
  ...
  def pad(a, length=512):
    ...
    if seq.size >= length:
      return seq[:length]
    pad_width = length - seq.size
    return np.concatenate([seq, np.zeros(pad_width, dtype=seq.dtype)])
