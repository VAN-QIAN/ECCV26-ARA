...
DEFAULT_IBA_AUGMENTED1_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/IBA_augmented_method1.jsonl")
)
DEFAULT_IBA_AUGMENTED2_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/IBA_augmented_method2.jsonl")
)
DEFAULT_ECHOSIGHT_ANCHOR_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/EchoSight_anchor.jsonl")
)
DEFAULT_ECHOSIGHT_AUGMENTED1_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/EchoSight_augmented_method1.jsonl")
)
DEFAULT_ECHOSIGHT_AUGMENTED2_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/EchoSight_augmented_method2.jsonl")
)
DEFAULT_WIKIPRF_ANCHOR_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/Wiki_PRF_anchor.jsonl")
)
DEFAULT_WIKIPRF_AUGMENTED1_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/Wiki_PRF_augmented_method1.jsonl")
)
DEFAULT_WIKIPRF_AUGMENTED2_PATH = (
    str(REPO_ROOT / "outputs/raw_methods/infoseek/augmented/Wiki_PRF_augmented_method2.jsonl")
)
DEFAULT_OUTPUT_DIR = str(REPO_ROOT / "results/evaluation/infoseek/augmented")
...
def strip_augmented_infoseek_suffix(data_id: str) -> str:
    text = _safe_text(data_id)
    if not text:
        return ""

    # augmented InfoSeek ids usually look like:
    # infoseek_val_00012345__anchor / __method1 / __method1__with_position / __method2__without_position
    match = re.match(r"^(infoseek_[^_]+_\d+)(?:__.*)?$", text)
    if match:
        return match.group(1)

    # Fallback: strip everything after first "__" if present.
...
def align_prediction_data_ids_to_gt(
    ...
    for pred in raw_predictions:
        ...
        stripped_data_id = strip_augmented_infoseek_suffix(source_data_id)
...
def score_predictions(
    ...
    type_total_gt: Dict[str, int] = {}
    type_matched: Dict[str, int] = {}
    type_correct_all_gt: Dict[str, int] = {}
    type_correct_on_matched: Dict[str, int] = {}

    total_gt = len(gt_ids)
    matched = 0
    non_empty = 0
    correct_all_gt = 0
    correct_on_matched = 0
    ...
    for data_id in gt_ids:
        ...
        type_total_gt[qtype] = type_total_gt.get(qtype, 0) + 1
        ...
        if pred_entry is not None:
            matched += 1
            type_matched[qtype] = type_matched.get(qtype, 0) + 1
            if non_empty_prediction:
                non_empty += 1

        score, processed_prediction = score_method_prediction(example, pred_entry)
        score_int = int(score)
        correct_all_gt += score_int
        type_correct_all_gt[qtype] = type_correct_all_gt.get(qtype, 0) + score_int
        if available:
            correct_on_matched += score_int
            type_correct_on_matched[qtype] = (
                type_correct_on_matched.get(qtype, 0) + score_int
            )
    ...
    per_type = {}
    for qtype, gt_count in type_total_gt.items():
        matched_count = type_matched.get(qtype, 0)
        hit_matched = type_correct_on_matched.get(qtype, 0)
        hit_all_gt = type_correct_all_gt.get(qtype, 0)
        per_type[qtype] = {
            # Accuracy denominator only counts data_ids that appear in this method's predictions.
            "correct": hit_matched,
            "total": matched_count,
            "accuracy": (hit_matched / matched_count) if matched_count else 0.0,
            "total_gt": gt_count,
            "accuracy_over_all_gt": (hit_all_gt / gt_count) if gt_count else 0.0,
        }
    ...
    summary = {
        ...
        "overall": {
            # Accuracy denominator only counts data_ids that appear in this method's predictions.
            "correct": correct_on_matched,
            "total": matched,
            "accuracy": (correct_on_matched / matched) if matched else 0.0,
            "total_gt": total_gt,
            "accuracy_over_all_gt": (correct_all_gt / total_gt) if total_gt else 0.0,
        },
        "coverage": {
            "matched_gt_ids": matched,
            "total_gt_ids": total_gt,
            "matched_ratio": (matched / total_gt) if total_gt else 0.0,
...
def main() -> None:
    ...
    qid2example, gt_ids = load_ground_truth(args.ground_truth_csv)
    if int(args.max_samples) > 0:
        gt_ids = gt_ids[: int(args.max_samples)]
        qid2example = {data_id: qid2example[data_id] for data_id in gt_ids}
    print(f"Loaded GT: {len(gt_ids)} rows from {args.ground_truth_csv}")
