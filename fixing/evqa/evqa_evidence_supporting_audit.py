...
def default_prompt_path():
    return os.path.join(os.path.dirname(__file__), "evqa_evidence_supporting_prompt.txt")
...
def default_output_path():
    return os.path.join("results_evidence_supporting", "evqa_evidence_supporting.jsonl")


def default_output_csv_path():
    return os.path.join("results_evidence_supporting", "evqa_evidence_supporting.csv")
...
def audit_and_parse(
    ...
    content, used_evidence = audit_once(
        prompt_template,
        question,
        annotated_answer,
        optional_allowed_json,
    ...
    return evidence_tag, extraction_answer, matches, explanation, used_evidence, content


def main():
    parser = argparse.ArgumentParser(
        description="Audit EVQA evidence sufficiency and answer match with LLM."
    )
    parser.add_argument("--input-csv", required=True)
    parser.add_argument("--output-jsonl", default=default_output_path())
    ...
    parser.add_argument(
        "--skip-question-types",
        default="2_hop",
        help="Comma-separated question_type values to skip (default: 2_hop).",
    )
    ...
    parser.add_argument("--replace-pipe-evidence", action="store_true")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Include evidence in output (default).",
    )
    parser.add_argument(
        "--no-include-evidence",
        dest="include_evidence",
        action="store_false",
        help="Omit evidence from output.",
    )
    parser.add_argument(
        "--dump-raw",
        default="",
    ...
    with open(args.output_jsonl, out_mode, encoding="utf-8") as out_f, open(
        ...
        for idx, row in iter_rows(args.input_csv):
            ...
            evidence_csv = evidence_raw
            if evidence_csv and args.replace_pipe_evidence:
                evidence_csv = evidence_csv.replace("|", "\n\n")
            ...
            result = {
                ...
                "model": args.model,
                "evidence_source": evidence_used_source,
                "section_ids": section_ids,
                "evidence_section_title": section_titles,
