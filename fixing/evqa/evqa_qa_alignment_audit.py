...
def default_prompt_path():
    return os.path.join(os.path.dirname(__file__), "evqa_qa_alignment_prompt.txt")


def default_output_jsonl_path():
    return os.path.join("results_qa_alignment", "evqa_qa_alignment.jsonl")


def default_output_csv_path():
    return os.path.join("results_qa_alignment", "evqa_qa_alignment.csv")


def parse_response(content):
    parsed = extract_json_block(content)
    if not parsed:
        return "Aligned", "", "unparseable_response"
    tag = normalize_text(parsed.get("qa_alignment_tag"))
    ...
    if tag == "Aligned":
        revised = ""
    return tag, revised, reason


def main():
    parser = argparse.ArgumentParser(description="Audit QA alignment for EVQA.")
    ...
    with open(args.output_jsonl, out_mode, encoding="utf-8") as out_f, open(
        ...
        writer = None
        extra_cols = ["qa_alignment_tag", "revised_question", "qa_alignment_reason"]
        write_header = True
        ...
        for idx, row in iter_rows(args.input_csv):
            ...
            tag, revised_question, reason = parse_response(content)

            result = {
                "data_id": data_id,
                "qa_alignment_tag": tag,
                "revised_question": revised_question,
                "qa_alignment_reason": reason,
                "model": args.model,
                "question": question,
            ...
            row = dict(row)
            row["qa_alignment_tag"] = tag
            row["revised_question"] = revised_question
            row["qa_alignment_reason"] = reason
