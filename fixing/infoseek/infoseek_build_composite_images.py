...
def build_data_id_to_image_id(source_rows):
    mapping = {}
...
def get_infoseek_image_path(image_id, image_root):
    if not image_id:
        return ""
    jpg_path = os.path.join(image_root, image_id + ".jpg")
    jpeg_path = os.path.join(image_root, image_id + ".JPEG")
...
def main():
    parser = argparse.ArgumentParser(
        description="Build side-by-side composite images for challenging InfoSeek query rows."
    )
    parser.add_argument("--challenging-csv", required=True, help="CSV created by infoseek_generate_challenging_queries.py")
    parser.add_argument(
        "--source-csv",
        required=True,
        help="Source InfoSeek CSV containing data_id and dataset_image_ids (e.g., infoseek_final_recheck_Feb7.csv).",
    )
    ...
    parser.add_argument("--image-root", default=DEFAULT_INFOSEEK_IMAGE_ROOT, help="InfoSeek raw image folder root.")
    ...
    data_to_img = build_data_id_to_image_id(source_rows)
    ...
    for idx, row in enumerate(challenging_rows):
        ...
        anchor_image_id = data_to_img.get(anchor_data_id, "")
        method1_image_id = data_to_img.get(method1_data_id, "")
        method2_image_id = data_to_img.get(method2_data_id, "")

        anchor_image_path = get_infoseek_image_path(anchor_image_id, args.image_root)
        method1_pair_image_path = get_infoseek_image_path(method1_image_id, args.image_root)
        method2_pair_image_path = get_infoseek_image_path(method2_image_id, args.image_root)
