...
def default_prompt_path():
    return os.path.join(os.path.dirname(__file__), "infoseek_question_fix_prompt.txt")
...
def default_output_jsonl_path():
    return os.path.join("results_question_fix", "infoseek_question_fix.jsonl")


def default_output_csv_path():
    return os.path.join("results_question_fix", "infoseek_question_fix.csv")
...
def main():
    parser = argparse.ArgumentParser(
        description="Audit InfoSeek question clarity and suggest fixes."
    )
        ...
        for idx, row in iter_rows(args.input_csv):
            ...
            if all_zero:
                result = {
                    "data_id": data_id,
                    "question_clarity_tag": "",
                    "clarity_reason": "missing_evidence",
                    "suggested_question": "",
                    "short_evidence": "",
                ...
                row = dict(row)
                row["question_clarity_tag"] = ""
                row["clarity_reason"] = "missing_evidence"
                row["suggested_question"] = ""
                row["short_evidence"] = ""
                row["short_evidence_source"] = "none"
                row["short_evidence_section_id"] = ""
                row["short_evidence_section_title"] = ""
                row["skip_reason"] = "Missing_evidence"
    ...
    print(
        f"Processed {processed} rows, wrote {written} outputs, "
        f"skipped {skipped_missing} missing-evidence rows."
    )
