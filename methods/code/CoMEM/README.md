...
```bash
scripts/methods/run_comem.sh --dataset evqa --split fixed
scripts/methods/run_comem.sh --dataset evqa --split unfixed
scripts/methods/run_comem.sh --dataset infoseek --split fixed
scripts/methods/run_comem.sh --dataset infoseek --split unfixed
```

Defaults:

- environment: `CoMEM`
- checkpoint: `data/checkpoints/CoMEM`
- outputs: `outputs/generated_methods/CoMEM/<dataset>/<split>/`
- MDS inputs: `CoMEM-inference/{EVQA,infoseek}/Custom_test_full*`

The customized EVQA/InfoSeek entrypoints accept `--metadata_path` and
`--mds_dir`, so fixed/unfixed data can be selected without editing Python files.
