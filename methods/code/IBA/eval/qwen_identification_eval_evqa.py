...
def load_ground_truth(csv_path: Path) -> Tuple[Dict[str, Dict[str, str]], List[str]]:
    ...
    test_list, header = load_csv_data(str(csv_path))
    header_index = {name: idx for idx, name in enumerate(header)}

    required_fields = {"wikipedia_title"}
    missing = required_fields - set(header_index)
    if missing:
        raise ValueError(f"Missing columns {missing} in {csv_path}")

    dataset_idx = header_index.get("dataset_name")

    for idx, row in enumerate(test_list):
        if "data_id" in header_index:
            data_id = row[header_index["data_id"]]
        else:
            data_id = f"E-VQA_{idx}"

        dataset_name = row[dataset_idx] if dataset_idx is not None else "unknown"
