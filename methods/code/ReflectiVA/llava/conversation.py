...
class LazyTokenizer:
    ...
    def _load(self):
        if self._tokenizer is None:
            tokenizer_path = os.getenv("TOKENIZER_PATH") or self.tokenizer_path or "aimagelab/ReflectiVA"
            self._tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
        return self._tokenizer

    def apply_chat_template(self, *args, **kwargs):
        return self._load().apply_chat_template(*args, **kwargs)
...
# The tokenizer is needed only when a LLaMA-3 prompt is rendered. Keeping it
# lazy makes argparse/help and syntax checks independent of model downloads.
llama_tokenizer = LazyTokenizer(os.getenv("TOKENIZER_PATH"))
