...
def run_test(
    ...
    **kwargs
    ...
    for it, test_example in tqdm.tqdm(enumerate(test_list)):
        ...
        elif example["data_id"].startswith("E-VQA_"):
            data_id = example["data_id"]
        else:
        ...
        if kwargs["perform_qformer_reranker"]:
            ...
            with torch.cuda.amp.autocast():
                ...
                rerank_step = 256  # to calculate the embedding in iteration
                ...
                alpha_1 = 0.5
                alpha_2 = 1 - alpha_1
                # NOTE: I would like to store the raw echosight section scores for analysis
                raw_scores = scores.clone().detach().cpu()
                scores = (
            ...
            # keep raw scores aligned with reranked sections
            reranked_sections_full = [sections[i] for i in reranked_index]
            raw_scores_sorted = [raw_scores[i].item() for i in reranked_index]
            raw_scores_dedup = []
            reranked_sections = []
            seen_sections = set()
            for sec, sc in zip(reranked_sections_full, raw_scores_sorted):
                if sec in seen_sections:
                    continue
                seen_sections.add(sec)
                reranked_sections.append(sec)
                raw_scores_dedup.append(sc)
            ...
            if kwargs["save_result"]:
                ...
                retrieval_result[data_id]["section_scores_echosight_raw"] = raw_scores_dedup[:10]
